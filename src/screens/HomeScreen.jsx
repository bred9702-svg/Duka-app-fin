    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '16px 14px 8px', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
          <StatCard label="Money coming in" value={fmtKES(income)} sub={`${transactions.filter(t => t.classified && t.operation_type === 'sale').length} sales`} color="green" delay={0.05} />
          <StatCard label="Money going out" value={fmtKES(expenses)} sub={`${transactions.filter(t => t.classified && t.operation_type === 'expense').length} expenses`} color="red" delay={0.1} />
          <StatCard label="Customers owe you" value={fmtKES(totalOwed)} sub={`${customers.filter(c => (c.total_owed || 0) > 0).length} customers`} color="amber" delay={0.15} />
          <StatCard label="Needs attention" value={unclassifiedCount} sub={unclassifiedCount ? 'Tap to classify →' : 'All clear ✓'} color={unclassifiedCount ? 'red' : 'green'} delay={0.2} />
        </div>
