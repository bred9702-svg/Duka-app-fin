  ...
  return (
    <div style={{ flex: 1, width: '100%', padding: '16px 14px 24px', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <button type="button" onClick={() => navigate(-1)} aria-label="Go back" style={{ width: 36, height: 36, borderRadius: 11, border: '1px solid var(--glass-border)', background: 'var(--glass-fill-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
        ...
        <div style={{ marginTop: 32, padding: '30px 18px', borderRadius: 20, textAlign: 'center', background: 'linear-gradient(160deg, rgba(240,169,61,.16), rgba(255,255,255,.03))', border: '1px solid rgba(240,169,61,.30)' }}>
          ...
          <p style={{ margin: '16px 0 5px', fontSize: 10, fontWeight: 750, color: '#F0A93D', textTransform: 'uppercase', letterSpacing: '.10em' }}>Dukwise Pro</p>
          <h1 style={{ margin: 0, fontFamily: 'var(--font-display)', fontSize: 20, color: 'var(--text-hi)' }}>{title}</h1>
          <p style={{ margin: '10px auto 0', maxWidth: 310, fontSize: 12, lineHeight: 1.65, color: 'var(--text-low)' }}>Unlock deeper analysis, intelligent recommendations and advanced tools to understand and grow your shop.</p>
