...
const TABS = [
  ...
  { id: 'analytics', path: '/analytics', icon: 'barChart', label: 'Stats' },
        ...
        return (
          <div key={tab.id} onClick={() => navigate(tab.path)} style={{
            ...
            <div style={{ position: 'relative', display: 'inline-flex' }}>
              <Icon name={tab.icon} size={17} color={active ? '#F0A93D' : 'var(--text-low)'} />
              {tab.id === 'inbox' && unclassifiedCount > 0 && (
                <span style={{ position: 'absolute', top: -3, right: -5, width: 7, height: 7, background: '#FF6B5B', borderRadius: '50%', boxShadow: '0 0 6px rgba(255,107,91,0.7)' }} />
              )}
            </div>
            <span style={{ fontFamily: 'var(--font-display)', fontSize: 8, color: active ? '#F0A93D' : 'var(--text-low)', fontWeight: 600 }}>
              {tab.label}
            </span>
          </div>
        )
